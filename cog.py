...
class Predictor(BasePredictor):
    ...
    def setup(self) -> None:
        ...
        for model_name in available_models.keys():
            separator = Separator(
                    model=model_name,
                    device ="cuda" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu",                    shifts=1,  # Will be updated per request
                    overlap=0.25,  # Will be updated per request
                    split=True,  # Will be updated per request
                    verbose=False  # Disable progress bars for cleaner logs
                )
    ...
    def predict(
        ...
        model: str = Input(
            ...
            default="htdemucs",
            choices=["mdx", "mdx_extra", "mdx_q", "mdx_extra_q", "hdemucs_mmi", "htdemucs", "htdemucs_ft", "htdemucs_6s"]
        ),
        ...
        if model not in self.separators:
            raise ValueError(f"Model {model} not available. Available models: {list(self.separators.keys())}")
        ...
        if segment is not None:
            ...
            if segment > max_segment:
                print(f"⚠️  Warning: Segment {segment}s exceeds model limit {max_segment}s, using model maximum")
        ...
        if verbose:
            print(f"Parameters: shifts={shifts}, overlap={overlap}, split={split}, segment={segment}, jobs={jobs}")
            print(f"🖥️  {self._get_gpu_memory_info()}")
        
        # Perform separation
        separated = separator.separate_audio_file(audio)
        ...
        for stem in stems_requested:
            if stem.startswith("no_"):
                ...
                if base_stem in available_stems:
                    ...
                    print(f"⚠️  Warning: Cannot create '{stem}' - base stem '{base_stem}' not available in model")
            elif stem not in available_stems:
                print(f"⚠️  Warning: Stem '{stem}' not available in model. Available: {', '.join(available_stems)}")
        ...
        for stem_name in valid_stems:
            ...
            if output_format == "wav":
                ...
                audio_bytes = separated.export_stem(stem_name, format=output_format, clip=clip_mode_enum)
